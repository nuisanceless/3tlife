...
public class ForgetPasswordActivity extends BaseActivity {
    ...
    void getVerificationCode(String account) {
        BaseResponse response = _restClient.getForgetPasswordVerifyCode(account);
    ...
    @Background
    void verifyCode() {
        BaseResponse response = _restClient.verifyForgetPasswordAuthCode(_verifyCode);
        if (BaseResponse.hasError(response)) {
            notifyFailed(BaseResponse.getErrorMessage(response));
        }
        else {
    ...
    void verifySuccessful() {
        SetPasswordActivity_.intent(this)._phone(_account)._register(false).startForResult(REQUEST_CODE_SET_PASSWORD);
    }
}
